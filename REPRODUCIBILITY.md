...
[шесть видео](docs/VIDEOS.md). Браузер/PDF-viewer/видеоплеер достаточны.
Копии media внутри отчёта сохранены как часть принятого комплекта; их байты идентичны корневым.
Текущий PDF направляет на главную страницу репозитория, а не запускает локальные файлы.
Git хранит одинаковые blobs один раз. CSV/рисунки сохранены побайтово; PDF и пояснения отредактированы без изменения результатов.
...
XeLaTeX либо Tectonic и настоящий Times New Roman; эти системные зависимости
не входят в Python venv. В текущем предоставленном PDF Times New Roman встроен;
36 страниц отрендерены и просмотрены. Локальная пересборка этой редакции не
проверялась: Times New Roman здесь отсутствует. Готовый PDF сохранён побайтово,
без подмены шрифта. Историческая QA прежних редакций сохранена отдельно.
